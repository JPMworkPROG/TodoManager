...
import { DemandNotFoundError, DemandConflictError, DemandValidationError } from '../errors/DemandErrors';
import logger from '../../utils/logger';

export class UpdateDemandUseCase {
   constructor(private readonly demandRepository: IDemandRepository) {}

   async execute(demandId: string, dto: UpdateDemandDTO, requestId?: string): Promise<Demand> {
      ...
      const demandWithSameTitle = await this.demandRepository.findByTitle(dto.title);
      ...
      if (endDate < existingDemand.startDate) {
         logger.warn('Demand update failed: invalid end date', {
            requestId,
            action: 'UPDATE_DEMAND',
            demandId,
            endDate: dto.endDate,
            startDate: existingDemand.startDate,
         });
