...
export class DemandController {
   ...
   async update(req: Request, res: Response): Promise<void> {
      ...
      const demand = await this.updateDemandUseCase.execute(demandId, dto, req.requestId);
      const demandDTO = toDemandDTO(demand);
   ...
   async createItem(req: Request, res: Response): Promise<void> {
      ...
      const item = await this.createDemandItemUseCase.execute(demandId, dto, req.requestId);
      const itemDTO = toDemandItemDTO(item);

      res.status(201).location(`/api/demands/${demandId}/items/${item.sku}`).json(itemDTO);
   }
   ...
   async updateItem(req: Request, res: Response): Promise<void> {
      ...
      const item = await this.updateDemandItemUseCase.execute(itemId, dto, req.requestId);
      const itemDTO = toDemandItemDTO(item);
